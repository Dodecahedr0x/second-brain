...
5. Create the research note **stub** titled by `short_form`: `$VAULT_PATH/Research/<short_form>.md` — frontmatter `source_type: research` / `agent_processed: false` / `agent_generated: true` / `agent_last_touched: YYYY-MM-DDThh:mm:ssZ`, tags `#research #stub`, the full driving question as a `> [!question]` callout, and a `<!-- research in progress -->` marker. Check legacy `$VAULT_PATH/<short_form>.md` first to avoid duplicates. Record it in `## Active Session` as `**Note**: [[<short_form>]]` (the durable, linkable home for the session).
